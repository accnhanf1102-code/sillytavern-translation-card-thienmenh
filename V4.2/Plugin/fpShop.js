...
(function () {
    ...
    // Helper checks mobile view
    function isMobileView() {
        try {
            if (window.parent && window.parent.matchMedia('(max-width: 768px)').matches) return true;
            if (parentDocument && parentDocument.body) {
                return parentDocument.body.clientWidth <= 768;
            }
...
@media (max-width: 768px) {
    ...
    .fps-header { height: 48px; padding: 0 14px; }

    .fps-content { flex-direction: column; overflow: hidden; }
    .fps-left { flex: 1; border: none; display: flex; flex-direction: column; overflow: hidden; }
    .fps-right { display: none; } /* Hide right panel completely, using bottom sheet instead */

    .fps-tabs { gap: 12px; padding: 0 12px; min-height: 46px; justify-content: space-around; overflow-x: auto; }
    .fps-tab { padding: 12px 10px; font-size: 11px; white-space: nowrap; flex: 1; margin: 0; }
    .fps-tab > span:not(.fps-tab-icon) { display: none; } /* Icon only on tight width, or keep text if desired. Actually keeping text is fine if flex:1 and small font */
    .fps-tab > span:not(.fps-tab-icon):last-child { display: inline; } /* wait, default has 2 span */

    .fps-grid-container { padding: 12px; }
    .fps-grid { grid-template-columns: 1fr; gap: 12px; }
    
    .fps-card { min-height: 90px; padding: 12px; }
    .fps-card-name { font-size: 15px; }

    .fps-state-screen.fps-empty-screen { padding: 16px; }
    .fps-icon-large { margin-bottom: 8px; font-size: 32px; }
    .fps-state-screen h2 { font-size: 18px; margin-bottom: 8px; }

    .fps-input-wrap { padding: 10px; flex-direction: row; gap: 8px; }
    .fps-input { flex: 1; min-height: 40px; font-size: 16px; } /* 16px prevents iOS zoom */
    .fps-send-btn { min-height: 40px; width: auto; padding: 0 20px; font-size: 13px; }

    .fps-loading-container { padding: 12px; }
    .fps-marker-preview { grid-template-columns: 1fr; gap: 16px; }
    .fps-marker-frame { height: 160px; width: 100%; border-radius: 8px; }
    .fps-marker-name { font-size: 18px; }
    .fps-marker-desc { font-size: 13px; overflow: hidden; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 3; }
    
    /* Hide some elements on mobile loading */
    .fps-img-nav { display: none; }
    .fps-marker-pagination { flex-direction: column; width: 100%; gap: 8px; }
    .fps-p-btn { width: 100%; }
    .fps-status-indicator { flex-direction: column; }
    .fps-ready-btn { width: 100%; padding: 14px; }
}
...
.fps-bottom-sheet-backdrop.show {
    opacity: 1;
    pointer-events: auto;
}
...
.fps-bottom-sheet {
    ...
    background: #0f141f; /* matching .fps-panel */
    border-top: 1px solid var(--fps-panel-edge);
...
.fps-bottom-sheet.show {
    transform: translateY(0);
}
.fps-bottom-sheet-handle {
    width: 36px; height: 4px; border-radius: 2px;
    background: rgba(255, 255, 255, 0.2);
    margin: 12px auto 8px;
}
.fps-bottom-sheet-content {
    flex: 1; overflow-y: auto; padding: 0 16px 20px;
}

/* Mobile Settings Dropdowns */
.fps-setting-item {
    display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px;
}
.fps-setting-item label {
    font-size: 13px; color: var(--fps-text-muted); text-transform: uppercase; letter-spacing: 0.1em;
}
.fps-setting-select {
    background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1);
    color: white; padding: 12px 14px; border-radius: 8px; font-size: 16px; outline: none;
}
.fps-setting-select option { background: #111; }
...
@media (min-width: 769px) {
    ...
    function createPanel() {
        ...
        // Panel nằm BÊN TRONG overlay để flexbox centering hoạt động
    ...
    function extractAndRepairJson(responseText) {
        ...
        // Sửa các lỗi AI điển hình gây gãy JSON
        // Thiếu dấu phẩy giữa các properties `"key": "value" "key2"` -> `"key": "value", "key2"`
        jsonStr = jsonStr.replace(/"\s+"/g, '", "'); 
        // Thiếu dấu ":" (Expected ':' after property name)
...
<div class="fps-right">
    ...
    function renderState() {
        ...
        let content = `
            <div class="fps-header">
                ...
                <div style="display:flex; align-items:center; gap:12px;">
                    <button class="fps-close-btn" id="fps-mobile-settings-btn" aria-label="Cài đặt">⚙</button>
        ...
        // Luôn render bottom sheets để tồn tại trong DOM (mô phỏng mobile realtime)
        content += renderMobileBottomSheets();

        panel.innerHTML = content;
        bindEvents();
    }

    function renderMobileBottomSheets() {
        const detail = state.selectedItem;
        const detailContent = detail ? renderDetailContent(detail) : '';

        return `
            <!-- Detail Sheet -->
            <div class="fps-bottom-sheet-backdrop ${state.isMobileDetailOpen ? 'show' : ''}" id="fps-detail-backdrop"></div>
            <div class="fps-bottom-sheet ${state.isMobileDetailOpen ? 'show' : ''}" id="fps-detail-sheet">
                <div class="fps-bottom-sheet-handle"></div>
                <div class="fps-bottom-sheet-content">
                    ${detailContent}
                </div>
                <div class="fps-buy-footer">
                    <button class="fps-buy-btn" id="fps-btn-buy-mobile" ${!detail || state.fpBalance < detail.cost ? 'disabled' : ''}>
            ...
            <!-- Settings Sheet -->
            <div class="fps-bottom-sheet-backdrop ${state.isMobileSettingsOpen ? 'show' : ''}" id="fps-settings-backdrop"></div>
            <div class="fps-bottom-sheet ${state.isMobileSettingsOpen ? 'show' : ''}" id="fps-settings-sheet">
                <div class="fps-bottom-sheet-handle"></div>
                <div class="fps-bottom-sheet-content">
                    <div style="font-size:18px; font-weight:700; color:white; margin-bottom:20px;">Cài đặt truy xuất</div>
                    <div class="fps-setting-item">
                        <label>Số Linh Phục Hệ Thống</label>
                        <select class="fps-setting-select" id="fps-worker-count-mobile">
                    ...
                    <div class="fps-setting-item">
                        <label>Mức độ mô tả</label>
                        <select class="fps-setting-select" id="fps-prompt-length-mobile">
    ...
    function renderLoadedState() {
        ...
        return `
            <div class="fps-content">
                <div class="fps-left">
                    <div class="fps-fp-bar" style="border-bottom: 1px solid var(--fps-panel-line);">
...
CÁC KỴ QUAN TRỌNG ĐỂ TRÁNH LỖI PHÂN TÍCH YAP/JSON:
1. TUYỆT ĐỐI KHÔNG thêm lời thừa vào trước hay sau block JSON. KHÔNG phản hồi bằng văn bản hội thoại (VD: "đây là vật phẩm của bạn...").
2. KHÔNG đặt JSON trong khối code markdown (VD: \`\`\`json ... \`\`\`). CHỈ TRẢ VỀ DUY NHẤT CHUỖI JSON HỢP LỆ sau khi đã thực hiện <thinking_personality>.
3. Đảm bảo đóng mở ngoặc '{', '}' và mảng '[', ']' chuẩn chỉnh.
4. TẤT CẢ các keys và các string values đều PHẢI ĐƯỢC BỌC BẰNG DẤU NHÁY KÉP ("). Không sử dụng nháy đơn hay để chuỗi tự do thiếu nháy.
5. TẤT CẢ thuộc tính phải có dấu NGĂN CÁCH HAI CHẤM (:) giữa Key và Value. Đừng bao giờ quên dấu (:) sau khi viết xong Key.
6. KHÔNG giải thích gì thêm sau khối JSON.
7. KHÔNG sử dụng ký tự xuống dòng thực tế bên trong các giá trị chuỗi. Sử dụng '\\n' để xuống dòng nếu cần.
</rule_output>
    ...
    function bindEvents() {
        ...
        panel.querySelectorAll('.fps-card').forEach(el => {
            el.onclick = () => {
                ...
                if (isMobileView()) {
                    state.isMobileDetailOpen = true;
                    // Manual update to allow CSS transition instead of full re-render
                    const detailContent = renderDetailContent(state.selectedItem);
                    const sheetContent = panel.querySelector('#fps-detail-sheet .fps-bottom-sheet-content');
                    if (sheetContent) sheetContent.innerHTML = detailContent;
                    
                    const buyBtnMobile = panel.querySelector('#fps-btn-buy-mobile');
                    if (buyBtnMobile) {
                        buyBtnMobile.disabled = state.fpBalance < state.selectedItem.cost;
                        buyBtnMobile.innerText = state.fpBalance >= state.selectedItem.cost ? 'MUA VẬT PHẨM' : 'KHÔNG ĐỦ FP';
                    }
                    
                    const backdrop = panel.querySelector('#fps-detail-backdrop');
                    const sheet = panel.querySelector('#fps-detail-sheet');
        ...
        const buyBtnMobile = panel.querySelector('#fps-btn-buy-mobile');
        if (buyBtnMobile) {
            buyBtnMobile.onclick = () => {
                if (!buyBtnMobile.disabled) purchaseItem(state.selectedItem);
            };
        }

        // Mobile UI Events
        const detailBackdrop = panel.querySelector('#fps-detail-backdrop');
        const settingsBackdrop = panel.querySelector('#fps-settings-backdrop');
        const mobileSettingsBtn = panel.querySelector('#fps-mobile-settings-btn');
        
        if (detailBackdrop) detailBackdrop.onclick = () => {
            state.isMobileDetailOpen = false;
            panel.querySelector('#fps-detail-backdrop')?.classList.remove('show');
            panel.querySelector('#fps-detail-sheet')?.classList.remove('show');
        };
        if (settingsBackdrop) settingsBackdrop.onclick = () => {
            state.isMobileSettingsOpen = false;
            panel.querySelector('#fps-settings-backdrop')?.classList.remove('show');
            panel.querySelector('#fps-settings-sheet')?.classList.remove('show');
        };
        if (mobileSettingsBtn) mobileSettingsBtn.onclick = () => {
            state.isMobileSettingsOpen = true;
            panel.querySelector('#fps-settings-backdrop')?.classList.add('show');
            panel.querySelector('#fps-settings-sheet')?.classList.add('show');
        };

        const wcMobile = panel.querySelector('#fps-worker-count-mobile');
        if (wcMobile) wcMobile.onchange = (e) => state.selectedWorkerCount = parseInt(e.target.value);
        
        const plMobile = panel.querySelector('#fps-prompt-length-mobile');
        if (plMobile) plMobile.onchange = (e) => state.selectedPromptLength = e.target.value;
    }
